...
import asyncio
import json
from config import DoorMonitorConfig
...
class Main:
    ...
    async def monitor_door(self, interval):
        handler = GpioHandler(self)
        await handler.start_monitoring_door(interval)
    ...
    async def gather_message_and_route(self,websocket):
        ...
        if message == "passive_data":
            data_holder = {"open_status":self.open}
    ...
    def name_and_type(self):
        data = {"name":self.config.name , "type":"reed_switch"}
        return json.dumps(data)

    def insert_alert_status(self,data_holder):
        if self.open == True:
            data_holder["alert_status"] = "alert_present"
            data_holder["message"] = f"{self.config.name} - Door Open!!"
        else:
